...
class SemPersistedLoader
  
  attr_accessor :ambient_temp
  attr_accessor :breathing_rate
  attr_accessor :br_conf
  attr_accessor :ecg1
  attr_accessor :ecg2
  attr_accessor :ecg_qual
  attr_accessor :end_time
  attr_accessor :heart_rate
  attr_accessor :hr_conf
  attr_accessor :lateral
  attr_accessor :longitudinal
  attr_accessor :rr_interval
  ...
  ###############################################################################
  def add_line( measured_at, tokens )
  
    hr_conf = 0
    hr_conf = tokens[11].to_i # if ! tokens[11].nil?
    rate = tokens[7].to_i
    if (hr_conf >= 70) && (rate > 40)
      min = measured_at.min
      sec = measured_at.sec

      @heart_beat[ min ] = {} if @heart_beat[ min ].nil?
      @heart_beat[ min ][ sec ] = [] if @heart_beat[ min ][ sec ].nil?
      @heart_beat[ min ][ sec ] << rate
  ...
  ###############################################################################
  def medium_rate( heart_rates )
    return nil if heart_rates.nil? || (heart_rates.size < 1)
    
    sorted_rates = heart_rates.sort
    return sorted_rates[ heart_rates.size / 2 ]
  end  # medium_rate
    ...
    for min in start_time.min..end_time.min do
      for sec in start_time.sec..end_time.sec do
        if @heart_beat[ min ].nil? || @heart_beat[ min ][ sec ].nil? || (@heart_beat[ min ][ sec ].size < 1)
          ...
          heart_beats << medium_rate( @heart_beat[ min ][ sec ] ).to_i
        end  # if
      end  # for
    end  # do
  ...
  def get_heart_rates( start_time, end_time )
    ...
    for min in start_time.min..end_time.min do
      if @heart_rate[ min ].nil?
        ...
        heart_rates << medium_rate( @heart_rate[ min ] ).to_i
      end  # if
    end  # do
